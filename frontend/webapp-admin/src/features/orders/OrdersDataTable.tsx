...
export function OrdersDataTable() {
  ...
  const { isLoading, data: initialOrders, error } = useOrders();
  const [orders, setOrders] = useState<Order[]>([]);

  // Initialize orders with data from useOrders
  useEffect(() => {
    if (initialOrders) {
  ...
  const columns: ColumnDef<Order>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className={`w-24 rounded-full px-2 py-1 text-center capitalize ${
